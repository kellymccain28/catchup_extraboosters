...
# Outputs for task 
orderly_artefact(
  'Produces a raw model run then a processed run for each row in the parameter dataset - summarized overall by age or by age/year',
  c("raw_modelrun.rds",
...
if(PEVstrategy == 'none' | PEVstrategy == 'AB'){
  orderly_artefact(
    'Produces a processed run for each row in the parameter dataset - summarized over last 15 years only if AB or no vaccine',
    c("processed_run_last15.rds")
  )
}
